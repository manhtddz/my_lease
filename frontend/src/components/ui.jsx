...
export function ErrorBox({ error, onRetry }) {
  ...
  return (
    <div className="card border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">
      ...
      <div className="mt-1 whitespace-pre-line">{error.message || String(error)}</div>
      {onRetry && (
...
export function Field({ label, error, hint, children, className = '' }) {
  return (
    <div className={className}>
      ...
      <div className={error ? '[&_input]:border-rose-400 [&_select]:border-rose-400' : ''}>{children}</div>
...
export function Badge({ tone = 'slate', children }) {
  return (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${TONE[tone]}`}>
      {children}
    </span>
  )
}

/** Màu theo mã trạng thái hoá đơn (CHAR(1)). */
export const INVOICE_TONE = { 1: 'slate', 2: 'sky', 3: 'amber', 4: 'green', 5: 'rose' }
export const ROOM_TONE = { 1: 'slate', 2: 'green', 3: 'amber' }
export const CONTRACT_TONE = { 1: 'slate', 2: 'green', 3: 'slate', 4: 'rose' }
        ...
        {items.map((t) => (
          <div
            ...
            className={`card px-4 py-3 text-sm shadow-lg ${
              t.tone === 'rose' ? 'border-rose-200 bg-rose-50 text-rose-800' : ''
            } ${t.tone === 'green' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : ''} ${
              t.tone === 'sky' ? 'border-sky-200 bg-sky-50 text-sky-800' : ''
            }`}
          >
